        ...
        if(!empty($data['luggage'])){
            ...
            foreach ($data['luggage'] as $index => $prod){
                ...
                if($order_info['shipping_type'] == '2'){
                    ...
                    $international_price = $this->Manage_price_model->get_international($country_id, $prods['product_id'], $carrier['id']);

                    $special_handling = 0;
                    if(!empty($international_price[0]['international_fee'])){
                        $special_handling = $international_price[0]['international_fee'];
                    }
                    ...
                    foreach($this->config->item('special_box_sizes_ranges') as $range){
                        ...
                        if($charge_weight >= $range['from'] && $charge_weight <= $range['to']){
                            ...
                            if($order_info['shipping_type'] == '2'){
                                ...
                                $special_handling = $special_fee_array[strtolower($range['size'])][$carrier['id']];
                                $special_handling_array[] = $special_handling;
                            }
...
//
    ...
    public function ax_save_order_item_list(){
        ...
        foreach($items_prices as $index => $single){

            if(empty($single) || $single <= 0 || empty($items_counts[$index]) || $items_counts[$index] <= 0){
                $data['errors'][] = 'All fields is required and can not be null.';
                echo json_encode($data);
                return false;
            }
    ...
    public function ax_fill_fields(){
        ...
        $state = $this->Order_model->get_states_by_name($state);

        if(empty($state)){

            $state = $this->Order_model->get_states_by_short_name($state);
        }
