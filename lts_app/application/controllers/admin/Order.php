...
Class Order extends CI_Controller
{
    ...
    public function ax_charge_or_refund()
    {
        ...
        if ($operation['type'] == 2 && $operation['amount'] > 0) {
            ...
            foreach ($payment_history as $single) {
                ...
                if ($single['card_number'] == 'From credit') {
                    ...
                    if ($new_amount == 0) {
                        ...
                        $this->Order_model->delete_payment_history($order_id, $single['id']);
            ...
            if ($refund_amount > 0 && $operation['amount'] > 0) {
                ...
                if ($error_refund_amount > 0) {
                    ...
                    $this->Order_model->update_payment_history(['amount' => $refund_amount], ['id' => $operation['id']]);
                }
    ...
    public function ax_save_order_item_list()
    {
        ...
        if (empty($items_names) || empty($items_counts) || empty($items_prices)) {
            $data['errors'][] = 'All fields is required.';
            echo json_encode($data);
            return false;
        }
...
//
    ...
    public function _create_shipment($order_id, $user_id)
    {
        ...
        $order_extra['signature_confirmation'] = 'INDIRECT';

        if (!empty($receiver_info['without_signature']) || stripos($order_info['currier_name'], 'fedex') === FALSE) {
...
//
    ...
    public function ax_save_final_billing()
    {
        ...
        foreach ($luggage_ids as $single_id) {
            ...
            if (empty($costs[$single_id]) && empty($actual_weights[$single_id]) && empty($actual_widths[$single_id]) && empty($actual_heights[$single_id]) && empty($actual_lengths[$single_id])) {
                continue;
            }

            $single_data['order_id'] = $order_id;
            $single_data['order_lug_id'] = $single_id;
            $single_data['actual_width'] = $actual_widths[$single_id];
            $single_data['actual_height'] = $actual_heights[$single_id];
            $single_data['actual_length'] = $actual_lengths[$single_id];
            $single_data['actual_weight'] = $actual_weights[$single_id];
            $single_data['cost'] = $costs[$single_id];
            ...
            if (count($single_data))
                ...
                $charge_weight = max(ceil($actual_weights[$single_id]), ceil($actual_widths[$single_id] * $actual_heights[$single_id] * $actual_lengths[$single_id] / 139));
