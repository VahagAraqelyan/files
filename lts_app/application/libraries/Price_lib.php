...
class Price_lib
{
    ...
    public function _international_shipping($country_from, $country_to, $date, $luggages, $special_boxes){
        ...
        $special_boxes = $this->_get_special_box_fee($special_boxes, 'international', $check_result['main_country']['id']);
        ...
        $international_fee   = $this->_get_international_fee($check_result['main_country']['id'], $luggages);
    ...
    public function _get_international_fee($country_id, $luggages){
        ...
        foreach($curriers_array as $single){
            ...
            foreach($luggages_id as $single_lug){
                if(!empty($luggages_fee[$single_lug])){
                    $fee += $luggages[$single_lug]*$luggages_fee[$single_lug];
                }
            }
    ...
    public function _get_special_box_fee($boxes, $type, $country_id){
        ...
        elseif(strtolower($type) == 'international'){
            $prices = $this->_get_box_inter($curriers, $boxes, $country_id);
        }
    ...
    public function _get_box_inter($curriers, $boxes, $country_id){
            ...
            foreach($boxes_info['boxes'] as $box){
                ...
                foreach($this->CI->config->item('special_box_sizes_ranges') as $range){
                    ...
                    if($box['weight'] >= $range['from'] && $box['weight'] <= $range['to']){
                        $prices[$currier['currier_name'].'_special'] += $international_fee[strtolower($range['size'])][$currier['id']]*$box['count'];
                        break;
                    }
