...
function get_item_list() {
    ...
    var sucScript  = '$("#signature").jSignature();$(".select-country").selectpicker("refresh")';
    send_ajax(url, 'post', send_data, {answer:'#account_information', success:sucScript,abort:true});
 }
...
$(document).on('click','.fill_butt_dhl',function () {
    var url = base_url+'order/ax_fill_fields';
    var order_id = $('#order_id').val();
    send_ajax(url, 'post', {state:state,order_id:order_id,city:city,address:address,zip_code:zip}, {handler:'ax_fill_fields_handler'});
});
...
function ax_fill_fields_handler(data) {
    ...
    if (obj_upload['errors'].length > 0) {
       ...
       setTimeout(function () {
           $('.edit_sender_info').trigger('click');
       },1000);
    }
}
