...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
                ...
                $this->insert_last_login_inf($user->id);
	...
	**/
	...
	// insert login date and ip

    public function insert_last_login_inf($user_id){
      ...
      //  $country = $this->get_country_by_iso($location['country']);

	    if(!empty($location)){


            $insert_data['country'] = $location['country'];
            $insert_data['city']    = $location['city'];
        }


	    return $this->db->insert('lts_login_history', $insert_data);

    }

   public function get_location($ip = NULL){

       if(base_url() != 'https://www.luggagetoship.com/'){
       ...
       $url = 'http://api.ipstack.com/'.$ip.'?access_key=35ca046cbfc3a03e15ad21fa2b9c432e';
       $respons = [];
       ...
       if(empty($content) || (empty($content->country_name) && empty($content->city))){
           return false;
       }


       $respons['country'] = $content->country_name;
       $respons['city'] = $content->city;
